...
def _is_required_check(name: str) -> bool:
    return name not in {"repodossier cli", "pipx"}
...
def _check_category(name: str) -> str:
    ...
    if name.startswith("git "):
        return "git"
    if name in {"python", "pytest", "repodossier cli", "pipx"}:
        return "tooling"
    if name in {"c runner", "r runner", "workflow rules", "workflow rules validator"}:
