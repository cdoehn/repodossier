...
def test_ai_exporter_is_hooked_into_dependency_section() -> None:
    ai_exporter_text = Path('src/repocontext/exporters/ai.py').read_text(encoding="utf-8")

    assert "append_dependencies_ai_section" in ai_exporter_text
    assert "_REPOCONTEXT_DEPENDENCY_AI_EXPORT_WRAPPER" in ai_exporter_text
    assert "_repocontext_wrap_ai_export_function" in ai_exporter_text
