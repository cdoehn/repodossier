...
def render_full_export_with_optional_model(
    export_or_context: object,
    *,
    use_model_renderer: bool = False,
    legacy_renderer: object | None = None,
) -> str:
    """Render a full export through the legacy path or explicit model renderer opt-in.

    The default keeps legacy behavior delegated to the provided legacy_renderer.
    The RepositoryExport renderer is only used when use_model_renderer=True.
    This prepares a controlled migration without changing existing callers.
    """
    ...
    if use_model_renderer:
        ...
        return render_full_export_from_model(export_or_context)
