"""Command-line interface entrypoint for RepoDossier."""
...
from repodossier.config import ConfigError, load_config_from_args, set_active_config, with_config_arguments


from repodossier._version import get_version
...
def _determine_version() -> str:
    ...
    try:
        return metadata.version("repodossier")
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--version",
        action="version",
        version=f"repodossier {get_version()}",
    )
...
def _repodossier_cli_repository_root() -> object:
...
def _mask_known_exports_after_cli_command(argv: object = None) -> None:
    ...
    repo_root = Path(_repodossier_cli_repository_root())
...
def _load_config_for_cli_args(arguments):
    """Load RepoDossier config for subcommands that expose config options."""
    ...
    setattr(arguments, "repodossier_config", config)
...
def main(*args: object, **kwargs: object) -> object:
    ...
    return result
_REPODOSSIER_DEPENDENCY_FULL_EXPORT_HOOK = True


def _repodossier_add_dependencies_to_full_export(full_text, local_values):
    """Add dependency information to full.txt text right before it is written."""

    if not isinstance(full_text, str):
        return full_text

    repo_root = _repodossier_dependency_repo_root_from_locals(local_values)
    files = _repodossier_dependency_files_from_locals(local_values)

    return append_dependencies_full_section(full_text, repo_root, files=files)


def _repodossier_dependency_repo_root_from_locals(local_values):
    ...
    for key in (
        ...
        candidate = _repodossier_dependency_path_from_value(value)
    ...
    if files:
        ...
        for file_item in files:
            path_value = _repodossier_dependency_path_from_value(file_item)
...
def _repodossier_dependency_files_from_locals(local_values):
    for key in (
        "files",
        "scanned_files",
        "file_infos",
        ...
        value = local_values.get(key)
        files = _repodossier_dependency_files_from_value(value)
...
def _repodossier_dependency_files_from_value(value):
    ...
    path_like_count = 0
    for item in items:
        if _repodossier_dependency_path_from_value(item) is not None:
            path_like_count += 1
...
def _repodossier_dependency_path_from_value(value):
    from pathlib import Path as _Path
